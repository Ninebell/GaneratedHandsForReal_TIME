...
#     folder_list = load_image_pair("D:\\GeoConGAN\\SynthHands_Release")
#     for path in folder_list:
#         print(path)
#
#     counts = len(folder_list)
#     for idx, path in enumerate(folder_list[:counts//10*8]):
#         origin = cv2.imread(path,cv2.IMREAD_COLOR)
#         hsv = cv2.cvtColor(origin,cv2.COLOR_BGR2HSV)
#         _, mask = cv2.threshold(hsv[:,:,1], 200,255,cv2.THRESH_BINARY)
#         mask = 255-mask
#
#         mask_3 = cv2.cvtColor(mask,cv2.COLOR_GRAY2RGB)
#         masked = np.bitwise_and(origin, mask_3)
...
#             h_l = math.floor(256 * rate)
#         back_real = np.zeros((256, 256, 3))
#         crop = cv2.resize(masked[y:h + y, x:x + w], (w_l, h_l))
#         back_real[256 - h_l:256, (256 - w_l) // 2:(256 + w_l) // 2] = crop
#
#         back_mask = np.zeros((256, 256))
#         back_mask[256 - h_l:256, (256 - w_l) // 2:(256 + w_l) // 2] = cv2.resize(white[y:y + h, x:x + w], (w_l, h_l))
#
#         cv2.imshow("origin", back_real)
#         cv2.imshow("mask", back_mask)
#         cv2.waitKey(1)
#         cv2.imwrite(os.path.join("D:\\GeoConGAN\\Data\\train_1\\origin","origin_{0}.png".format(idx+39425)), back_real)
#         cv2.imwrite(os.path.join("D:\\GeoConGAN\\Data\\train_1\\mask","mask_{0}.png".format(idx+39425)), back_mask)
#
#     for idx, path in enumerate(folder_list[counts//10*8:]):
#
#         print(idx, ": ", idx//(counts-counts//10*8) * 100)
#         origin = cv2.imread(path,cv2.IMREAD_COLOR)
#         hsv = cv2.cvtColor(origin,cv2.COLOR_BGR2HSV)
#
#         _, mask = cv2.threshold(hsv[:,:,1], 200,255,cv2.THRESH_BINARY)
...
#         cv2.imshow("mask", back_mask)
#         cv2.waitKey(1)
#         cv2.imwrite(os.path.join("D:\\GeoConGAN\\Data\\test_1\\origin","origin_{0}.png".format(idx+9866)), back_real)
#         cv2.imwrite(os.path.join("D:\\GeoConGAN\\Data\\test_1\\mask","mask_{0}.png".format(idx+9866)), back_mask)
#
